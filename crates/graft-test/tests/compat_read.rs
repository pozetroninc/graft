...
fn read_upstream_data_from_fs_remote() {
    ...
    let volume = runtime.volume_open(None, None, Some(log_id.clone())).unwrap();
    ...
    let commit = runtime.get_commit(&log_id, lsn!(1)).unwrap()
    ...
    assert!(vol.leaf_hash_min_lsn.is_none(),
